...
from src.models.value_based.doubleqlearning.rl_tools_doubleqlearning import update_Q_doubleQlearning
...
def main(args, logger):
    ...
    # Parametrisation
    params = json.loads(open('src/models/value_based/doubleqlearning/config/{}.json'.format(method_id)).read())
    ...
    Q1 = init_Q(n_actions, params)
    Q2 = init_Q(n_actions, params)
    Q_saved = Q1.copy()
    ...
    while (not evo_training['convergence']) & (evo_training['episode'] < params['n_episodes']):
        ...
        while (not evo_episode['done']) and (evo_episode['n_episode_steps'] < params['nmax_steps']):
            action1 = epsilon_greedy(Q1, state1['state_id'], n_actions, epsilon, Q2)
            ...
            if rng.uniform(0, 1) < 0.5:
                ...
                # Learning the Q-value
                Q1 = update_Q_doubleQlearning(Q1, Q2, state1['state_id'], action1, reward1,
                                              state2['state_id'], action2, method_params)
            else:
                # Choosing the next action
                action2 = select_best_action(Q2[state2['state_id']])
                # Learning the Q-value
                Q2 = update_Q_doubleQlearning(Q2, Q1, state1['state_id'], action1, reward1,
                                              state2['state_id'], action2, method_params)
        ...
        # save_models({'Q1': Q1, 'Q2': Q2, 'evo_training': evo_training}, method_id, final=False)
        #     os.system("python -m src.models.run_one_episode value_based/doubleqlearning/config/{} {}".format(
        ...
        if (evo_training['episode'] + 1) % n_episodes_save == 0:
            ...
            #    evo_training['checking'], Q_saved, Q1, method_id, info_params, final=False)
            # save_models({'Q1': Q1, 'Q2': Q2, 'evo_training': evo_training}, method_id, final=False)
    ...
    _, _ = launch_checking(
        evo_training['checking'], Q_saved, Q1, method_id, info_params, final=True)
    save_models({'Q1': Q1, 'Q2': Q2, 'evo_training': evo_training}, method_id, final=True)
    ...
    logger.info('episode')
    os.system("python -m src.models.run_one_episode value_based/doubleqlearning/config/{} {} --save_episode --plot_episode".format(
        method_id, 'final'))

    # display_analyse_Q(Q1, Q2, method_id, info_params, final=True)
...
# python -m src.models.value_based.doubleqlearning.train_model_DoubleQLearning doubleqlearning_test --update_episode_division 1
