...
def main(args, logger):
    ...
    # Parametrisation
    params = json.loads(open('src/models/value_based/sarsa/config/{}.json'.format(method_id)).read())
    ...
    while (not evo_training['convergence']) & (evo_training['episode'] < params['n_episodes']):
        ...
        #     os.system("python -m src.models.run_one_episode value_based/sarsa/config/{} {}".format(
    ...
    logger.info('episode')
    os.system("python -m src.models.run_one_episode value_based/sarsa/config/{} {} --save_episode --plot_episode".format(
        method_id, 'final'))
...
# python -m src.models.value_based.sarsa.train_model_SARSA sarsa_test --update_episode_division 10
